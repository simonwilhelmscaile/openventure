...
export function Footer({ content, companyName }: FooterProps) {
  return (
    <footer className="border-t border-[var(--border-default)] bg-[var(--bg-primary)] py-16">
      <div className="mx-auto max-w-[1200px] px-6">
        <div className="grid gap-12 md:grid-cols-2 lg:grid-cols-5">
          ...
          {content.columns.map((column, index) => (
            <div key={index}>
              <div className="mb-4 text-sm font-semibold uppercase tracking-wide text-[var(--text-secondary)]" role="heading" aria-level={2}>
                {column.title}
              </div>
              <ul className="space-y-0">
                {column.links.map((link, linkIndex) => (
                  <li key={linkIndex}>
                    <a
                      href={link.href}
                      className="text-sm text-[var(--text-secondary)] transition-colors hover:text-[var(--text-primary)] inline-flex items-center h-[44px]"
                    >
                      {link.text}
                    </a>
                  </li>
        ...
        <div className="mt-16 flex flex-col items-center justify-between gap-4 border-t border-[var(--border-default)] pt-8 md:flex-row">
          ...
          <div className="flex gap-6">
            {content.bottom_links.map((link, index) => (
              <a
                key={index}
                href={link.href}
